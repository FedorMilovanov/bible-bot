...
from battle_integrity import get_pending_final_battles
from legacy_delivery_worker import deliver_battle_recipient_once, deliver_report_once
from report_integrity import get_pending_reports
...
async def drain_pending_deliveries(
    *,
    ...
    if not isinstance(reports, list) or not isinstance(battles, list):
        raise LegacyDeliveryQueueInvalid("pending delivery listing returned invalid data")
    ...
    for report in reports:
        ...
        try:
            ...
            photo_sent, text_sent = await deliver_report_once(report_id, report_photo_sender, report_text_sender)
    ...
    for battle in battles:
        ...
        for participant_id in participants:
            try:
                sent = await deliver_battle_recipient_once(battle_id, participant_id, battle_sender)
    ...
    return DeliveryDrainSummary(
        reports_seen=len(reports), report_stage_sends=report_stage_sends,
        battles_seen=len(battles), battle_recipient_sends=battle_recipient_sends,
        deferred=deferred, errors=tuple(errors),
    )
