...
def test_drain_delivers_reports_and_battle_recipients(monkeypatch):
    ...
    monkeypatch.setattr(drain, "get_pending_final_battles", lambda limit: [{"_id": "b1", "creator_id": 10, "opponent_id": 20}])
    ...
    result = run(drain.drain_pending_deliveries(
        battle_sender=_noop_battle, report_photo_sender=_noop_report, report_text_sender=_noop_report, limit=7
    ))
...
def test_item_failure_does_not_starve_later_queue_entries(monkeypatch):
    seen = []
    monkeypatch.setattr(drain, "get_pending_reports", lambda _limit: [{"_id": "bad"}, {"_id": "good"}])
    ...
    monkeypatch.setattr(drain, "deliver_report_once", report_once)
    result = run(drain.drain_pending_deliveries(
        battle_sender=_noop_battle, report_photo_sender=_noop_report, report_text_sender=_noop_report
    ))
    assert seen == ["good"]
...
def test_existing_leases_are_deferred_not_errors(monkeypatch):
    ...
    monkeypatch.setattr(drain, "get_pending_final_battles", lambda _limit: [{"_id": "b1", "creator_id": 10, "opponent_id": 20}])
...
def test_listing_failure_propagates_fail_closed(monkeypatch):
    monkeypatch.setattr(drain, "get_pending_reports", lambda _limit: (_ for _ in ()).throw(RuntimeError("mongo unavailable")))
    monkeypatch.setattr(drain, "get_pending_final_battles", lambda _limit: [])
    with pytest.raises(RuntimeError, match="mongo unavailable"):
        run(drain.drain_pending_deliveries(
            battle_sender=_noop_battle, report_photo_sender=_noop_report, report_text_sender=_noop_report
        ))
...
def test_limit_validation_happens_before_listing(monkeypatch):
    ...
    with pytest.raises(ValueError, match="limit"):
        run(drain.drain_pending_deliveries(
            battle_sender=_noop_battle, report_photo_sender=_noop_report, report_text_sender=_noop_report, limit=0
        ))
