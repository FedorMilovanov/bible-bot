...
def test_chapter3_uses_learning_only_policy():
...
def test_regular_result_router_sends_chapter3_to_learning_path(monkeypatch):
    captured = {}
    ...
    receipt = result_store.apply_regular_result_once(
        user_id=7,
        result_id="chapter3-learning-result",
        username="learner",
        first_name="Learner",
        level_key="chapter3",
        score=8,
    ...
    assert captured["level_key"] == "chapter3"
...
def test_learning_persistence_updates_progress_not_ranking_totals(monkeypatch):
    captured = {}
    ...
    receipt = result_store._apply_learning_result_once(
        user_id=7,
        result_id="chapter3-progress-only",
        username="learner",
        first_name="Learner",
        level_key="chapter3",
        score=9,
    ...
    assert captured["update"]["$max"] == {"chapter3_best_score": 9}
