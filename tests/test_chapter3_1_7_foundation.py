...
def test_morphgnt_machine_fixture():
    tags = "D--------- V-PPPNPF- V-3PAI-P-- N-----DSM- N-----GSF- N-----DSM- N-----NSM- A-----GSN- A-----GSN- V-3IAI-P-- V-3AAI-S-- V-PAPNSF- V-PMPNPF- N-----ASF- V-PAPNPM- N-----ASF- A-----DSNC N-----DSN- A-----DSN- A-----DPM- V-PPN---- N-----APF-".split()
    morph = g.MORPHGNT_EVIDENCE_1P3
    assert len(morph) == 22
    assert [row["tag"] for row in morph.values()] == tags
    assert morph["ὑποτασσόμεναι_3_1"]["lemma"] == "ὑποτάσσω"
    assert morph["ἀσθενεστέρῳ"]["lemma"] == "ἀσθενής"


def test_explicit_boundaries_and_semantic_control():
    cards = {x["id"]: x for x in G}
    for n in (103, 105, 107, 109, 111, 115, 119):
