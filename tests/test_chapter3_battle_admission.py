...
def test_challenge_taxonomy_and_fallback_remain_chapter1_only():
    chapter3_ids = _ids(CHAPTER3_REVIEWED_QUESTIONS)
...
def test_seeded_challenge_selection_never_leaks_chapter3():
    chapter3_ids = _ids(CHAPTER3_REVIEWED_QUESTIONS)
    for mode in ("random20", "hardcore20"):
        for seed in range(128):
            ...
            assert chapter3_ids.isdisjoint(selected_ids), (mode, seed)


def test_chapter3_normal_learning_stays_non_scoring_after_battle_admission():
...
def test_battle_admission_manifest_is_exact_and_challenge_closed():
