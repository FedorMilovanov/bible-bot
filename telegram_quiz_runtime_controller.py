# ruff: noqa: RUF001
"""Canonical production Telegram quiz runtime.

This module contains only the quiz runtime reachable from ``telegram_production``.
It deliberately does not import ``bot.py`` or the historical giant
``telegram_controller.py``. Mongo remains durable authority; process-local
mappings come from ``telegram_quiz_runtime_state``.
"""
...
from course_catalog import SURFACE_TELEGRAM, legacy_level_config
...
from quiz_answer_history import build_progress_bar, is_wrong
from session_integrity import QuizSessionAnswerConflict, QuizSessionStoreUnavailable
from telegram_answer_animation import animate_answer_buttons
from telegram_conversation_states import ANSWERING
import telegram_main_menu as main_menu
from telegram_quiz_result_menu import send_final_results_menu
from telegram_quiz_runtime_state import (
    create_session_data,
    get_user_data,
    get_user_lock,
    reset_bad_input,
...
async def _run_blocking_io(function: Callable[..., _T], /, *args, **kwargs) -> _T:
...
def _hydrate_session(
    ...
    *,
    ...
    """Rebuild process-local/UI fields from a validated durable session."""
...
async def _show_active_attempt(target: Any, session: dict, *, edit: bool) -> None:
    ...
    actions = (
        "Продолжить или отменить повторение ошибок?"
        if session.get("is_retry") is True
        else "Продолжить, начать заново или отменить текущую попытку?"
    )
    text = (
...
async def _launch_attempt(
    *,
    ...
    question_ids = [get_qid(question) for question in questions]
    try:
        outcome = await _run_blocking_io(
...
async def _send_current_question(bot, user_id: int, prefix: str) -> None:
    ...
    progress = build_progress_bar(index + 1, len(questions), data.get("answered_questions", []))
    text = (
    ...
    try:
        canonical_sent_at = await _run_blocking_io(
            lambda: mark_live_question_sent(user_id, data, target, sent_at=sent_at)
        )
    except (
        LegacyLiveAnswerStale,
        LegacyLiveQuestionStateInvalid,
...
async def resume_session_handler(update: Update, context):
    ...
    user_id = query.from_user.id
    try:
        resolved = await _run_blocking_io(lambda: resolve_session_action(query.data, "res", user_id))
...
async def cancel_session_handler(update: Update, context):
    ...
    user_id = query.from_user.id
    try:
        resolved = await _run_blocking_io(lambda: resolve_session_action(query.data, "can", user_id))
    except LegacySessionActionUnavailable:
        await query.answer("⚠️ База сессий временно недоступна.", show_alert=True)
        return
    except LegacySessionActionStale:
    ...
    await query.edit_message_text("❌ Тест отменён.", reply_markup=main_menu.main_keyboard())


async def _cancel_current(user_id: int) -> tuple[bool, str]:
    try:
        result = await _run_blocking_io(lambda: cancel_current_incomplete_session(user_id))
...
async def reset_session_inline(update: Update, context):
    ...
    if status in {"unavailable", "conflict"}:
        ...
        return
    await safe_edit(query, "🆘 Тест сброшен.", reply_markup=main_menu.main_keyboard())


async def _status_session(user_id: int):
    try:
        session = await _run_blocking_io(lambda: get_active_quiz_session_strict(user_id))
...
async def status_command(update: Update, context):
    ...
    if status == "none":
        await update.message.reply_text("📌 Нет активного теста.", reply_markup=main_menu.main_keyboard())
        return
...
async def show_status_inline(update: Update, context):
    ...
    if status == "unavailable":
        ...
        return
    if status == "conflict":
        await safe_edit(query, "⚠️ Состояние сессии противоречиво; ничего не удалено.")
        return
    if status == "finalize":
...
async def start(update: Update, context):
    ...
    if context.args:
        ...
        config = legacy_level_config(surface=SURFACE_TELEGRAM)
        if level_key in config:
            cfg = config[level_key]
            keyboard = InlineKeyboardMarkup([
                [InlineKeyboardButton("🧘 Без ограничения времени", callback_data=f"relaxed_mode_{level_key}")],
                [InlineKeyboardButton(f"⏱ На время ({TIMED_MODE_TIMEOUT} сек)", callback_data=f"timed_mode_{level_key}")],
                [InlineKeyboardButton(f"⚡ Скоростной ({SPEED_MODE_TIMEOUT} сек)", callback_data=f"speed_mode_{level_key}")],
...
async def _save_all_sessions(_application=None):
    """Cancel process-local timers only; Mongo state is authoritative."""
    cancelled = 0
