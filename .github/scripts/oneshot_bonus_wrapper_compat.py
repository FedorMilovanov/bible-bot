...
old_daily = '''def claim_daily_bonus_once(user_id: int, day: str, daily_streak: int | None = None) -> dict:
...
'''
new_daily = '''def claim_daily_bonus_once(user_id: int, day: str, daily_streak: int | None = None) -> int:
...
'''
old_challenge = '''def claim_challenge_bonus_once(user_id: int, mode: str, score: int, day: str) -> dict:
...
'''
new_challenge = '''def claim_challenge_bonus_once(user_id: int, mode: str, score: int, day: str) -> int:
    """Compatibility wrapper: return points only for the winning claim."""
    stage = claim_challenge_bonus_state(user_id, mode, score, day)
    return stage["bonus"] if stage["claimed_now"] else 0
'''

for label, old, new in (("daily", old_daily, new_daily), ("challenge", old_challenge, new_challenge)):
    if text.count(old) != 1:
        raise SystemExit(f"{label} wrapper exact block count={text.count(old)}")
