from keep_alive import keep_alive
keep_alive()

from telegram import Update, ReplyKeyboardMarkup, ReplyKeyboardRemove, InlineKeyboardButton, InlineKeyboardMarkup
from telegram.ext import Application, CommandHandler, MessageHandler, filters, ConversationHandler, CallbackQueryHandler
import json
import time
import os
from datetime import datetime
from pymongo import MongoClient

# --- НАСТРОЙКА БАЗЫ ДАННЫХ ---
MONGO_URL = os.getenv('MONGO_URL') 

if MONGO_URL:
    try:
        cluster = MongoClient(MONGO_URL)
        db = cluster["bible_bot_db"]
        collection = db["leaderboard"]
    except Exception as e:
        print(f"Ошибка подключения к БД: {e}")
        collection = None
else:
    print("⚠️ ВНИМАНИЕ: Не задана переменная MONGO_URL. Статистика не будет сохраняться!")
    collection = None

# Состояния разговора
CHOOSING_LEVEL, ANSWERING = range(2)

# ЛЁГКИЙ УРОВЕНЬ
easy_questions = [
    {
        "question": "Кто написал Первое послание Петра?",
        "options": ["Апостол Павел", "Апостол Петр", "Апостол Иоанн", "Апостол Иаков"],
        "correct": 1,
        "explanation": "Автором послания является апостол Петр, один из ближайших учеников Иисуса Христа. Он написал это послание около 62-63 гг. н.э., находясь в Риме."
    },
    {
        "question": "В каком году примерно было написано послание?",
        "options": ["30-33 гг.", "50-55 гг.", "62-63 гг.", "70-75 гг."],
        "correct": 2,
        "explanation": "Послание было написано около 62-63 гг. н.э., незадолго до начала массовых гонений при императоре Нероне, которые начались в 64 г. после пожара в Риме."
    },
    {
        "question": "Где находился Петр, когда писал послание?",
        "options": ["В Иерусалиме", "В Риме", "В Антиохии", "В Ефесе"],
        "correct": 1,
        "explanation": "Петр находился в Риме, когда писал послание. В 1 Пет. 5:13 он символически называет Рим 'Вавилоном', возможно, для безопасности христиан во время начинающихся гонений."
    },
    {
        "question": "Как Петр символически назвал Рим в послании?",
        "options": ["Египет", "Вавилон", "Содом", "Ниневия"],
        "correct": 1,
        "explanation": "Петр назвал Рим 'Вавилоном' (1 Пет. 5:13). Это было кодовое имя, символизирующее центр языческой власти и идолопоклонства, подобно древнему Вавилону."
    },
    {
        "question": "Кому было адресовано послание?",
        "options": ["Римским христианам", "Иерусалимской церкви", "Христианам Малой Азии", "Всем язычникам"],
        "correct": 2,
        "explanation": "Послание было адресовано христианам, рассеянным в провинциях Малой Азии: Понте, Галатии, Каппадокии, Асии и Вифинии (1 Пет. 1:1). Это были верующие, переживавшие локальные преследования."
    },
    {
        "question": "Какова главная цель послания?",
        "options": ["Осудить лжеучителей", "Укрепить в страданиях", "Объяснить доктрины", "Собрать пожертвования"],
        "correct": 1,
        "explanation": "Главная цель — укрепить верующих в страданиях, напомнить о будущей славе во Христе и призвать к святой жизни, послушанию и надежде на спасение."
    },
    {
        "question": "Через что Бог возродил нас к живому упованию? (1 Пет. 1:3)",
        "options": ["Через крещение", "Через воскресение Христа", "Через веру", "Через Слово Божье"],
        "correct": 1,
        "explanation": "Бог возродил нас 'воскресением Иисуса Христа из мёртвых к упованию живому' (1 Пет. 1:3). Наша надежда основана на воскресшем Христе, а не на земных обстоятельствах."
    },
    {
        "question": "Какое наследие ожидает верующих? (1 Пет. 1:4)",
        "options": ["Земное богатство", "Нетленное, чистое, неувядаемое", "Долгая жизнь", "Власть над народами"],
        "correct": 1,
        "explanation": "Верующих ожидает 'наследство нетленное, чистое, неувядаемое, хранящееся на небесах' (1 Пет. 1:4). Это не земное богатство, а вечная эсхатологическая надежда на новое небо и новую землю."
    },
    {
        "question": "Как верующие сохраняются ко спасению? (1 Пет. 1:5)",
        "options": ["Перенося верно страдания", "Силою Божией посредством их веры", "Через Причастие", "Добрыми делами"],
        "correct": 1,
        "explanation": "Верующие 'силою Божиею через веру соблюдаемы ко спасению' (1 Пет. 1:5). Бог силой сохраняет нас для вечного спасения — это дар Божий, а не наша заслуга."
    },
    {
        "question": "К чему призывает Петр верующих? (1 Пет. 1:15-16)",
        "options": ["К богатству", "К святости", "К терпению", "К знанию"],
        "correct": 1,
        "explanation": "'По примеру призвавшего вас Святого, и сами будьте святы во всех поступках. Ибо написано: будьте святы, потому что Я свят' (1 Пет. 1:15-16). Святость — это отделение от греха и подражание Божьей святости."
    }
]

# СРЕДНИЙ УРОВЕНЬ
medium_questions = [
    {
        "question": "Почему Петр назвал Рим 'Вавилоном'?",
        "options": ["Это метафора идолопоклонства, центра языческой власти", "Рим был основан вавилонянами", "Петр ошибся", "Он на самом деле и сидел в Вавилоне, а не в Риме"],
        "correct": 0,
        "explanation": "Вавилон — это кодовое имя для Рима. Во времена преследований авторы избегали прямых указаний на места и имена. Вавилон символизировал Рим из-за его идолопоклонства и языческой власти (ср. Откр. 17:18)."
    },
    {
        "question": "При каком императоре начались массовые гонения на христиан?",
        "options": ["Клавдий", "Нерон", "Домициан", "Траян"],
        "correct": 1,
        "explanation": "Массовые гонения начались при императоре Нероне в 64 г. н.э., после пожара Рима. Нерон обвинил христиан в поджоге и начал их систематическое преследование. Петр и Павел приняли мученическую смерть при Нероне."
    },
    {
        "question": "Что означает греческое слово 'πρόγνωσις' (предведение)?",
        "options": ["Простое предвидение", "Избрание по заветной любви", "Знание будущего", "Божья мудрость"],
        "correct": 1,
        "explanation": "Греческое 'πρόγνωσις' означает не просто предвидение, а избрание по заветной любви (1 Пет. 1:2). Это указывает на Божью инициативу в спасении — Он избрал нас по Своей любви ещё до создания мира."
    },
    {
        "question": "Кто исследовал пророчества о спасении? (1 Пет. 1:10)",
        "options": ["Апостолы", "Пророки", "Ангелы", "Первосвященники"],
        "correct": 1,
        "explanation": "'К сему-то спасению относились изыскания и исследования пророков' (1 Пет. 1:10). Пророки тщательно исследовали свои пророчества, но не знали времени исполнения (ср. Дан. 12:8)."
    },
    {
        "question": "Что указывал Дух Христов в пророках? (1 Пет. 1:11)",
        "options": ["Только страдания Христа", "Только славу Христа", "Страдания и последующую славу", "Конец мира"],
        "correct": 2,
        "explanation": "Дух Христов 'предвозвещал Христовы страдания и последующую за ними славу' (1 Пет. 1:11). Страдания Мессии и Его прославление — центральная тема пророчеств Ветхого Завета."
    },
    {
        "question": "Что означает 'препоясать чресла ума'? (1 Пет. 1:13)",
        "options": ["Молиться усерднее", "Быть собранным духовно, бодрствовать в мышлении", "Изучать Писание", "Поститься"],
        "correct": 1,
        "explanation": "'Препоясав чресла ума вашего, бодрствуя...' (1 Пет. 1:13) — образ готовности к действию, как при Исходе из Египта. Это призыв к духовной собранности, бодрствованию ума и готовности к духовной борьбе."
    },
    {
        "question": "С чем сравнивается испытание веры? (1 Пет. 1:7)",
        "options": ["С очищением серебра", "С очищением золота огнем", "С огранкой алмаза", "С закаливанием стали"],
        "correct": 1,
        "explanation": "'Испытанная вера ваша оказалась драгоценнее гибнущего, хотя и огнем испытываемого золота' (1 Пет. 1:7). Как золото очищается от примесей в огне, так вера испытывается и очищается через страдания."
    },
    {
        "question": "Откуда взята цитата 'будьте святы, потому что Я свят'?",
        "options": ["Исход", "Левит", "Второзаконие", "Псалмы"],
        "correct": 1,
        "explanation": "Цитата взята из книги Левит (11:44-45; 19:2; 20:7,26). Петр применяет призыв к святости из Ветхого Завета к христианам, показывая, что они — святой народ Божий."
    },
    {
        "question": "Что является целью веры? (1 Пет. 1:9)",
        "options": ["Богатство", "Здоровье", "Спасение душ", "Мудрость"],
        "correct": 2,
        "explanation": "'Достигая наконец верою вашею спасения душ' (1 Пет. 1:9). Конечная цель веры — не земное благополучие, а вечное спасение души, которое мы получим при явлении Христа."
    },
    {
        "question": "Кто желает проникнуть в тайну спасения? (1 Пет. 1:12)",
        "options": ["Демоны", "Ангелы", "Люди", "Пророки"],
        "correct": 1,
        "explanation": "'Во что желают проникнуть Ангелы' (1 Пет. 1:12). Греческое 'παρακύψαι' означает благоговейное любопытство. Даже ангелы с интересом исследуют тайну спасения, доступную верующим."
    }
]

# СЛОЖНЫЙ УРОВЕНЬ
hard_questions = [
    {
        "question": "Какие три действия Троицы описаны в 1 Пет. 1:2?",
        "options": [
            "Творение, искупление, освящение",
            "Предведение Отца, освящение Духа, окропление кровью Христа",
            "Избрание, призвание, прославление",
            "Вера, надежда, любовь"
        ],
        "correct": 1,
        "explanation": "'Избранным, по предведению Бога Отца, при освящении от Духа, к послушанию и окроплению Кровию Иисуса Христа' (1 Пет. 1:2). Спасение — троичное дело: Отец избирает, Дух освящает, Сын искупает."
    },
    {
        "question": "Что означает греческое слово 'παρακύψαι' (желают проникнуть) в 1 Пет. 1:12?",
        "options": [
            "Страх и трепет",
            "Благоговейное любопытство",
            "Глубокое исследование",
            "Недоумение"
        ],
        "correct": 1,
        "explanation": "Греческое 'παρακύψαι' буквально означает 'наклониться, чтобы заглянуть'. Это выражает благоговейное любопытство ангелов к тайне спасения. Даже небесные существа желают глубже понять Божий план искупления."
    },
    {
        "question": "Почему Петр использовал образ 'препоясания чресел'?",
        "options": [
            "Это была обычная одежда того времени",
            "Образ воинской готовности",
            "Образ готовности к Исходу из Египта",
            "Символ скромности"
        ],
        "correct": 2,
        "explanation": "Образ 'препоясания чресел' отсылает к Исходу (Исх. 12:11), когда израильтяне ели пасху, будучи готовы к немедленному выходу из Египта. Петр использует этот образ для призыва к духовной готовности и бодрствованию."
    },
    {
        "question": "Что Петр НЕ упомянул как характеристику небесного наследия?",
        "options": ["Нетленное", "Чистое", "Неувядаемое", "Безгрешное"],
        "correct": 3,
        "explanation": "Петр описывает наследие как 'нетленное, чистое, неувядаемое' (1 Пет. 1:4). Слово 'безгрешное' не используется, хотя 'нетленность' указывает на вечную ценность, подобную безгрешности Христа (Евр. 7:26)."
    },
    {
        "question": "Какое слово из Сираха 2:5 (неканоническая книга) перекликается с учением о страданиях в 1 Пет. 1:7?",
        "options": [
            "Золото испытывается в огне",
            "Терпение рождает опытность",
            "Страдание очищает душу",
            "Вера без дел мертва"
        ],
        "correct": 0,
        "explanation": "В Сир. 2:5 сказано: 'Золото испытывается в огне'. Петр использует тот же образ: вера испытывается страданиями, как золото огнём, чтобы очиститься от примесей и явить свою подлинность."
    },
    {
        "question": "В каком контексте Петр говорит о 'живой надежде'?",
        "options": [
            "Надежда на улучшение жизни",
            "Надежда основана на воскресшем Христе",
            "Надежда на избавление от страданий",
            "Надежда на второе пришествие"
        ],
        "correct": 1,
        "explanation": "'Возродивший нас воскресением Иисуса Христа из мертвых к упованию живому' (1 Пет. 1:3). 'Живая надежда' основана на историческом событии — воскресении Христа, а не на абстрактных ожиданиях или земных обстоятельствах."
    },
    {
        "question": "Как связаны страдания Христа и Его слава в пророчествах по 1 Пет. 1:11?",
        "options": [
            "Слава без страданий",
            "Страдания без славы",
            "Сначала страдания, потом слава",
            "Слава заменяет страдания"
        ],
        "correct": 2,
        "explanation": "Дух Христов предвозвещал 'Христовы страдания и последующую за ними славу' (1 Пет. 1:11). Пророчества показывают необходимую последовательность: сначала страдания Мессии, затем Его прославление (ср. Ис. 53; Пс. 21)."
    },
    {
        "question": "Какая привилегия верующих подчёркнута в 1 Пет. 1:12?",
        "options": [
            "Они умнее пророков",
            "Они живут в эпоху исполнения пророчеств",
            "Они не нуждаются в пророчествах",
            "Они знают больше ангелов"
        ],
        "correct": 1,
        "explanation": "'Им открыто было, что не им самим, а нам служило то...' (1 Пет. 1:12). Привилегия верующих в том, что они живут в эпоху исполнения пророчеств. Пророки желали увидеть то, что видят христиане."
    },
    {
        "question": "Что означает 'не сообразуйтесь с прежними похотями' в контексте святости?",
        "options": [
            "Полное безгрешие",
            "Отделение от греховных моделей прошлого",
            "Аскетический образ жизни",
            "Уход от мира"
        ],
        "correct": 1,
        "explanation": "'Не сообразуйтесь с прежними похотями, бывшими в неведении вашем' (1 Пет. 1:14). Это призыв отделиться от греховных моделей поведения прошлой жизни, как Израиль отделялся от обычаев Египта и Ханаана."
    },
    {
        "question": "Почему Бог сохраняет верующих 'силою Божией через веру' (1 Пет. 1:5)?",
        "options": [
            "Вера — человеческое достижение",
            "Сохранение — дар Божий, а не наша заслуга",
            "Бог помогает тем, кто помогает себе",
            "Вера заменяет Божью силу"
        ],
        "correct": 1,
        "explanation": "'Силою Божиею через веру соблюдаемых ко спасению' (1 Пет. 1:5). Мы сохраняемы ко спасению силою Божией, а не нашей. Вера — это дар Божий и средство, через которое Бог действует, а не наша самостоятельная заслуга."
    }
]

# Хранилище данных пользователей (текущая сессия)
user_data = {}

# --- ФУНКЦИИ БАЗЫ ДАННЫХ ---

def add_to_leaderboard(user_id, username, first_name, level_key, score, total, time_seconds):
    if collection is None: return

    points_per_question = {"easy": 1, "medium": 2, "hard": 3}
    earned_points = score * points_per_question[level_key]
    user_id_str = str(user_id)
    
    try:
        entry = collection.find_one({"_id": user_id_str})
        
        if entry:
            new_total = entry.get("total_points", 0) + earned_points
            new_attempts = entry.get(f"{level_key}_attempts", 0) + 1
            new_best_score = max(entry.get(f"{level_key}_best_score", 0), score)
            
            current_best_time = entry.get(f"{level_key}_best_time", float('inf'))
            new_best_time = min(current_best_time, time_seconds)
            
            collection.update_one(
                {"_id": user_id_str},
                {
                    "$set": {
                        "total_points": new_total,
                        f"{level_key}_attempts": new_attempts,
                        f"{level_key}_best_score": new_best_score,
                        f"{level_key}_best_time": new_best_time,
                        "last_date": datetime.now().strftime("%Y-%m-%d %H:%M"),
                        "first_name": first_name,
                        "username": username
                    }
                }
            )
        else:
            new_entry = {
                "_id": user_id_str,
                "username": username or "Без username",
                "first_name": first_name or "Пользователь",
                "total_points": earned_points,
                "easy_attempts": 1 if level_key == "easy" else 0,
                "medium_attempts": 1 if level_key == "medium" else 0,
                "hard_attempts": 1 if level_key == "hard" else 0,
                "easy_best_score": score if level_key == "easy" else 0,
                "medium_best_score": score if level_key == "medium" else 0,
                "hard_best_score": score if level_key == "hard" else 0,
                "easy_best_time": time_seconds if level_key == "easy" else float('inf'),
                "medium_best_time": time_seconds if level_key == "medium" else float('inf'),
                "hard_best_time": time_seconds if level_key == "hard" else float('inf'),
                "last_date": datetime.now().strftime("%Y-%m-%d %H:%M")
            }
            collection.insert_one(new_entry)
    except Exception as e:
        print(f"Ошибка записи в БД: {e}")

def get_user_position(user_id):
    if collection is None: return None, None
    user_id_str = str(user_id)
    try:
        entry = collection.find_one({"_id": user_id_str})
        if not entry:
            return None, None
        
        my_points = entry.get("total_points", 0)
        count_better = collection.count_documents({"total_points": {"$gt": my_points}})
        return count_better + 1, entry
    except Exception:
        return None, None

def get_top_10():
    if collection is None: return []
    try:
        return list(collection.find().sort("total_points", -1).limit(10))
    except Exception:
        return []

def format_time(seconds):
    if seconds == float('inf'):
        return "—"
    minutes = int(seconds // 60)
    secs = int(seconds % 60)
    if minutes > 0:
        return f"{minutes}м {secs}с"
    return f"{secs}с"

# Команда /start
async def start(update: Update, context):
    keyboard = [
        [InlineKeyboardButton("📖 О боте", callback_data='about')],
        [InlineKeyboardButton("🎯 Начать тест", callback_data='start_test')],
        [InlineKeyboardButton("🏆 Таблица лидеров", callback_data='leaderboard')],
        [InlineKeyboardButton("📊 Моя статистика", callback_data='my_stats')]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    
    await update.message.reply_text(
        '📖 *БИБЛЕЙСКИЙ ТЕСТ-БОТ*\n\n'
        '*Тема:* 1 Петра 1:1-16\n'
        '*Материал:* Введение и комментарий\n\n'
        'Выбери действие:',
        reply_markup=reply_markup,
        parse_mode='Markdown'
    )

# Обработка кнопок главного меню
async def button_handler(update: Update, context):
    query = update.callback_query
    await query.answer()
    
    if query.data == 'about':
        await query.edit_message_text(
            '📚 *О БОТЕ*\n\n'
            'Этот бот поможет проверить знания по Первому посланию Петра (глава 1).\n\n'
            '*Материал:*\n'
            '• Авторство и исторический контекст\n'
            '• Комментарий на 1 Пет. 1:1-16\n'
            '• Богословские концепции\n\n'
            '*Уровни сложности:*\n'
            '🟢 Лёгкий — основные факты (1 балл за вопрос)\n'
            '🟡 Средний — понимание контекста (2 балла за вопрос)\n'
            '🔴 Сложный — глубокое богословие (3 балла за вопрос)\n\n'
            '*Система баллов:*\n'
            '• Баллы суммируются за все прохождения\n'
            '• Общий рейтинг по набранным баллам\n'
            '• Статистика по каждому уровню\n\n'
            'Используй /test чтобы начать!',
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ Назад", callback_data='back_to_main')]]),
            parse_mode='Markdown'
        )
    
    elif query.data == 'start_test':
        await choose_level(update, context, is_callback=True)
    
    elif query.data == 'leaderboard':
        await show_general_leaderboard(query)
    
    elif query.data == 'my_stats':
        await show_my_stats(query)

# Показ общей таблицы лидеров
async def show_general_leaderboard(query):
    top = get_top_10()
    
    if not top:
        text = '🏆 *ОБЩАЯ ТАБЛИЦА ЛИДЕРОВ*\n\n'
        text += 'Пока никто не проходил тесты.\nБудь первым! 🚀'
    else:
        text = '🏆 *ОБЩАЯ ТАБЛИЦА ЛИДЕРОВ*\n\n'
        
        for i, entry in enumerate(top, 1):
            medal = ""
            if i == 1: medal = "🥇"
            elif i == 2: medal = "🥈"
            elif i == 3: medal = "🥉"
            
            name = entry.get('first_name', 'Unknown')
            if len(name) > 15:
                name = name[:15] + "..."
            
            text += f'{medal} *{i}.* {name}\n'
            text += f'   💎 {entry.get("total_points", 0)} баллов\n'
            
            attempts = []
            if entry.get("easy_attempts", 0) > 0: attempts.append(f'🟢{entry["easy_attempts"]}')
            if entry.get("medium_attempts", 0) > 0: attempts.append(f'🟡{entry["medium_attempts"]}')
            if entry.get("hard_attempts", 0) > 0: attempts.append(f'🔴{entry["hard_attempts"]}')
            
            if attempts:
                text += f'   Прохождений: {" ".join(attempts)}\n\n'
            else:
                text += '\n'
    
    # Кнопки навигации (чтобы не создавать новое сообщение)
    keyboard = [
        [InlineKeyboardButton("🎯 Начать тест", callback_data='start_test')],
        [InlineKeyboardButton("⬅️ Назад", callback_data='back_to_main')]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    
    await query.edit_message_text(text, reply_markup=reply_markup, parse_mode='Markdown')

# Моя статистика
async def show_my_stats(query):
    user_id = query.from_user.id
    
    position, entry = get_user_position(user_id)
    
    if not entry:
        text = '📊 *МОЯ СТАТИСТИКА*\n\n'
        text += 'Вы ещё не проходили тесты.\n\nИспользуйте /test чтобы начать!'
    else:
        text = '📊 *МОЯ СТАТИСТИКА*\n\n'
        text += f'🏅 Позиция в общем рейтинге: *#{position}*\n'
        text += f'💎 Всего баллов: *{entry.get("total_points", 0)}*\n\n'
        
        text += '📈 *КОЛИЧЕСТВО ПРОХОЖДЕНИЙ:*\n'
        text += f'🟢 Лёгкий: {entry.get("easy_attempts", 0)} раз(а)\n'
        text += f'🟡 Средний: {entry.get("medium_attempts", 0)} раз(а)\n'
        text += f'🔴 Сложный: {entry.get("hard_attempts", 0)} раз(а)\n\n'
        
        text += '⭐ *ЛУЧШИЕ РЕЗУЛЬТАТЫ:*\n'
        if entry.get("easy_attempts", 0) > 0:
            text += f'🟢 Лёгкий: {entry.get("easy_best_score")}/10 • {format_time(entry.get("easy_best_time"))}\n'
        if entry.get("medium_attempts", 0) > 0:
            text += f'🟡 Средний: {entry.get("medium_best_score")}/10 • {format_time(entry.get("medium_best_time"))}\n'
        if entry.get("hard_attempts", 0) > 0:
            text += f'🔴 Сложный: {entry.get("hard_best_score")}/10 • {format_time(entry.get("hard_best_time"))}\n'
        
        text += f'\n📅 Последняя активность: {entry.get("last_date", "?")}'
    
    # Кнопки навигации
    keyboard = [
        [InlineKeyboardButton("🎯 Начать тест", callback_data='start_test')],
        [InlineKeyboardButton("⬅️ Назад", callback_data='back_to_main')]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    
    await query.edit_message_text(text, reply_markup=reply_markup, parse_mode='Markdown')

# Возврат в главное меню
async def back_to_main(update: Update, context):
    query = update.callback_query
    await query.answer()
    
    keyboard = [
        [InlineKeyboardButton("📖 О боте", callback_data='about')],
        [InlineKeyboardButton("🎯 Начать тест", callback_data='start_test')],
        [InlineKeyboardButton("🏆 Таблица лидеров", callback_data='leaderboard')],
        [InlineKeyboardButton("📊 Моя статистика", callback_data='my_stats')]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    
    await query.edit_message_text(
        '📖 *БИБЛЕЙСКИЙ ТЕСТ-БОТ*\n\n'
        'Выбери действие:',
        reply_markup=reply_markup,
        parse_mode='Markdown'
    )

# Выбор уровня сложности
async def choose_level(update, context, is_callback=False):
    keyboard = [
        [InlineKeyboardButton("🟢 Лёгкий (1 балл за вопрос)", callback_data='level_easy')],
        [InlineKeyboardButton("🟡 Средний (2 балла за вопрос)", callback_data='level_medium')],
        [InlineKeyboardButton("🔴 Сложный (3 балла за вопрос)", callback_data='level_hard')],
        [InlineKeyboardButton("⬅️ Назад", callback_data='back_to_main')]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    
    text = (
        '🎯 *ВЫБЕРИ УРОВЕНЬ СЛОЖНОСТИ*\n\n'
        '🟢 *Лёгкий:* 1 балл за вопрос\n'
        'Основные факты, даты, имена\n\n'
        '🟡 *Средний:* 2 балла за вопрос\n'
        'Исторический контекст, значения терминов\n\n'
        '🔴 *Сложный:* 3 балла за вопрос\n'
        'Богословские концепции, греческие термины, глубокий анализ'
    )
    
    if is_callback and hasattr(update, 'callback_query'):
        await update.callback_query.edit_message_text(text, reply_markup=reply_markup, parse_mode='Markdown')
    else:
        # Вызвано командой /test
        await update.message.reply_text(text, reply_markup=reply_markup, parse_mode='Markdown')

# Начало теста выбранного уровня
async def level_selected(update: Update, context):
    query = update.callback_query
    await query.answer()
    
    user_id = update.effective_user.id
    
    if query.data == 'back_to_main':
        await back_to_main(update, context)
        return ConversationHandler.END
    
    # Выбор набора вопросов
    if query.data == 'level_easy':
        questions = easy_questions
        level_name = "🟢 Лёгкий"
        level_key = "easy"
    elif query.data == 'level_medium':
        questions = medium_questions
        level_name = "🟡 Средний"
        level_key = "medium"
    else:  # hard
        questions = hard_questions
        level_name = "🔴 Сложный"
        level_key = "hard"
    
    # Инициализация данных пользователя
    user_data[user_id] = {
        "questions": questions,
        "level_name": level_name,
        "level_key": level_key,
        "current_question": 0,
        "correct_answers": 0,
        "wrong_answers": [],
        "start_time": time.time()
    }
    
    await query.edit_message_text(
        f'*{level_name} уровень*\n\n'
        f'Начинаем тест! 📝\n'
        f'Засекаем время... ⏱',
        parse_mode='Markdown'
    )
    await send_question(query.message, user_id)
    return ANSWERING

# Отправка вопроса
async def send_question(message, user_id):
    data = user_data[user_id]
    q_num = data["current_question"]
    
    if q_num >= len(data["questions"]):
        await show_results(message, user_id)
        return ConversationHandler.END
    
    q = data["questions"][q_num]
    keyboard = [[option] for option in q["options"]]
    
    await message.reply_text(
        f'*Вопрос {q_num + 1}/10*\n\n{q["question"]}\n\nВыбери правильный ответ:',
        reply_markup=ReplyKeyboardMarkup(keyboard, one_time_keyboard=True, resize_keyboard=True),
        parse_mode='Markdown'
    )

# Обработка ответа
async def answer(update: Update, context):
    user_id = update.effective_user.id
    
    if user_id not in user_data:
        await update.message.reply_text("Используй /test чтобы начать тест")
        return ConversationHandler.END
    
    data = user_data[user_id]
    q_num = data["current_question"]
    q = data["questions"][q_num]
    
    user_answer = update.message.text
    
    # Проверяем ответ
    try:
        answer_index = q["options"].index(user_answer)
        
        if answer_index == q["correct"]:
            data["correct_answers"] += 1
        else:
            data["wrong_answers"].append({
                "question": q["question"],
                "your_answer": user_answer,
                "correct_answer": q["options"][q["correct"]],
                "explanation": q["explanation"]
            })
        
        # Молча принимаем ответ без подсказок
        await update.message.reply_text("✓ Принято", reply_markup=ReplyKeyboardRemove())
        
    except ValueError:
        await update.message.reply_text("Пожалуйста, выбери один из предложенных вариантов")
        return ANSWERING
    
    # Следующий вопрос
    data["current_question"] += 1
    
    if data["current_question"] < len(data["questions"]):
        await send_question(update.message, user_id)
        return ANSWERING
    else:
        await show_results(update.message, user_id)
        return ConversationHandler.END

# Показ результатов
async def show_results(message, user_id):
    data = user_data[user_id]
    score = data["correct_answers"]
    total = len(data["questions"])
    percentage = (score / total) * 100
    
    time_taken = time.time() - data["start_time"]
    user = message.from_user
    
    # Добавляем в БД
    add_to_leaderboard(
        user_id,
        user.username,
        user.first_name,
        data["level_key"],
        score,
        total,
        time_taken
    )
    
    position, entry = get_user_position(user_id)
    
    # Очки за уровень
    points_per_question = {"easy": 1, "medium": 2, "hard": 3}
    earned_points = score * points_per_question[data["level_key"]]
    
    if percentage >= 90:
        grade = "Отлично! 🌟"
        emoji = "🏆"
    elif percentage >= 70:
        grade = "Хорошо! 👍"
        emoji = "✅"
    elif percentage >= 50:
        grade = "Удовлетворительно 📖"
        emoji = "📝"
    else:
        grade = "Нужно повторить материал 📚"
        emoji = "📖"
    
    result_text = f'{emoji} *РЕЗУЛЬТАТЫ ТЕСТА*\n\n'
    result_text += f'*Уровень:* {data["level_name"]}\n'
    result_text += f'*Правильных ответов:* {score} из {total}\n'
    result_text += f'*Процент:* {percentage:.0f}%\n'
    result_text += f'*Заработано баллов:* +{earned_points} 💎\n'
    result_text += f'*Всего баллов:* {entry.get("total_points", 0)} 💎\n'
    result_text += f'*Время:* {format_time(time_taken)}\n'
    result_text += f'*Позиция в рейтинге:* #{position}\n'
    result_text += f'*Оценка:* {grade}\n\n'
    
    if data["wrong_answers"]:
        result_text += '❌ *РАЗБОР ОШИБОК:*\n\n'
        for i, wrong in enumerate(data["wrong_answers"], 1):
            result_text += f'*{i}. {wrong["question"]}*\n'
            result_text += f'   ❌ Ваш ответ: {wrong["your_answer"]}\n'
            result_text += f'   ✅ Правильно: {wrong["correct_answer"]}\n'
            result_text += f'   📝 {wrong["explanation"]}\n\n'
    else:
        result_text += '🎉 *Все ответы правильные!*\n\n'
    
    # ВАЖНО: Добавляем кнопки ВНИЗУ результатов
    # чтобы не надо было скроллить вверх
    keyboard = [
        [InlineKeyboardButton("🔄 Пройти снова", callback_data='start_test')],
        [InlineKeyboardButton("🏆 Таблица лидеров", callback_data='leaderboard')],
        [InlineKeyboardButton("⬅️ Главное меню", callback_data='back_to_main')]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    
    await message.reply_text(result_text, reply_markup=reply_markup, parse_mode='Markdown')

# Команда /test
async def test_command(update: Update, context):
    await choose_level(update, context, is_callback=False)
    return CHOOSING_LEVEL

# Команда /leaderboard
async def leaderboard_command(update: Update, context):
    top = get_top_10()
    
    if not top:
        text = '🏆 *ОБЩАЯ ТАБЛИЦА ЛИДЕРОВ*\n\n'
        text += 'Пока никто не проходил тесты.\nБудь первым! 🚀'
    else:
        text = '🏆 *ОБЩАЯ ТАБЛИЦА ЛИДЕРОВ*\n\n'
        for i, entry in enumerate(top, 1):
            medal = ""
            if i == 1: medal = "🥇"
            elif i == 2: medal = "🥈"
            elif i == 3: medal = "🥉"
            
            name = entry.get('first_name', 'Unknown')
            if len(name) > 15: name = name[:15] + "..."
            
            text += f'{medal} *{i}.* {name}\n'
            text += f'   💎 {entry.get("total_points", 0)} баллов\n'
            # (код про попытки скрыт для краткости, он тот же)
            attempts = []
            if entry.get("easy_attempts", 0) > 0: attempts.append(f'🟢{entry["easy_attempts"]}')
            if entry.get("medium_attempts", 0) > 0: attempts.append(f'🟡{entry["medium_attempts"]}')
            if entry.get("hard_attempts", 0) > 0: attempts.append(f'🔴{entry["hard_attempts"]}')
            if attempts: text += f'   Прохождений: {" ".join(attempts)}\n\n'
            else: text += '\n'
    
    # Кнопки для команды /leaderboard
    keyboard = [
        [InlineKeyboardButton("🎯 Начать тест", callback_data='start_test')],
        [InlineKeyboardButton("⬅️ Назад", callback_data='back_to_main')]
    ]
    await update.message.reply_text(text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='Markdown')

# Отмена
async def cancel(update: Update, context):
    keyboard = [[InlineKeyboardButton("⬅️ Главное меню", callback_data='back_to_main')]]
    await update.message.reply_text(
        '❌ Тест отменён.',
        reply_markup=InlineKeyboardMarkup(keyboard)
    )
    return ConversationHandler.END

# Главная функция
def main():
    app = Application.builder().token("8134773553:AAF4DWLR7DBDolkigso_ZgXd4Ml_90YaaK8").build()
    
    # Обработчик теста
    conv_handler = ConversationHandler(
        entry_points=[
            CommandHandler('test', test_command),
            CallbackQueryHandler(level_selected, pattern='^level_')
        ],
        states={
            CHOOSING_LEVEL: [CallbackQueryHandler(level_selected)],
            ANSWERING: [MessageHandler(filters.TEXT & ~filters.COMMAND, answer)]
        },
        fallbacks=[
            CommandHandler('cancel', cancel),
            CallbackQueryHandler(back_to_main, pattern='^back_to_main$')
        ],
        allow_reentry=True
    )
    
    app.add_handler(conv_handler)
    app.add_handler(CommandHandler("start", start))
    app.add_handler(CommandHandler("leaderboard", leaderboard_command))
    app.add_handler(CallbackQueryHandler(button_handler, pattern='^(about|start_test|leaderboard|my_stats)$'))
    app.add_handler(CallbackQueryHandler(back_to_main, pattern='^back_to_main$'))
    
    print('🤖 Библейский тест-бот запущен!')
    app.run_polling()

if __name__ == '__main__':
    main()

